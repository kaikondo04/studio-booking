...
export default function BookingForm() {
  ...
  const [type, setType] = useState('normal') // ★ここに 'personal' (個人練) が加わる！
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ★ バリデーション（種類の違いで必須項目を変える！）
      if (type === 'personal') {
        if (!leader) {
          alert('名前を入力してください！')
          return
        }
        // 【重要】不正なハッキング（デベロッパーツール等）での未来予約をここで完全に弾く！
        if (date > tomorrow) {
      ...
      if (type === 'personal') finalBandName = '個人練' // 個人練の場合はバンド名を自動入力！
      ...
      <form onSubmit={handleSubmit} className="mb-8 p-6 bg-white border border-gray-300 rounded-xl shadow-sm">
        ...
        <div className="grid gap-6">
          ...
          <div>
            ...
            {/* ★ 5つのボタンを綺麗に並べるためにレイアウトを調整！ */}
            <div className="grid grid-cols-2 gap-2 mb-2">
              <button
                type="button"
                onClick={() => { setType('normal'); setIsAllDay(false); }}
                className={`py-3 px-2 rounded-lg font-bold text-sm transition border-2 ${
                  type === 'normal' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'
                }`}
              >
                通常練習
              </button>
              <button
                ...
                onClick={() => { 
                  setType('personal'); 
                  setIsAllDay(false);
                  // ★ もしすでに明後日以降が選ばれていたら、今日に強制リセットする！
                  if (date > tomorrow) setDate(today);
                }}
                className={`py-3 px-2 rounded-lg font-bold text-sm transition border-2 ${
                  type === 'personal' ? 'bg-green-600 text-white border-green-600' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'
                }`}
              >
                個人練習 (緑)
              </button>
            ...
            <div className="grid grid-cols-2 gap-2 mb-2">
              <button
                ...
                className={`py-3 px-2 rounded-lg font-bold text-sm transition border-2 ${
                  type === 'event' ? 'bg-purple-600 text-white border-purple-600' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'
                }`}
              >
                告知のみ(枠なし)
              </button>
              <button
                ...
                className={`py-3 px-2 rounded-lg font-bold text-sm transition border-2 ${
                  type === 'live' ? 'bg-red-500 text-white border-red-500' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'
                }`}
              >
                ライブ(赤)
              </button>
            ...
            <div className="grid grid-cols-1 gap-2">
              <button
                ...
                onClick={() => setType('ng')}
                className={`py-3 px-2 rounded-lg font-bold text-sm transition border-2 ${
                  type === 'ng' ? 'bg-gray-600 text-white border-gray-600' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'
                }`}
              >
                使用不可
              </button>
          ...
          {type !== 'personal' && (
            <div>
              ...
              <input 
                ...
                className="w-full p-3 text-lg border-2 border-gray-300 rounded-lg text-black bg-white focus:ring-2 focus:ring-blue-500 outline-none"
          ...
          <div>
            <label className="block font-bold mb-2 text-black">
               {type === 'personal' ? '名前（個人練に入る人）' : type === 'normal' ? '代表者' : '登録者'}
            </label>
            <input 
              type="text" 
              value={leader}
              ...
              className="w-full p-3 text-lg border-2 border-gray-300 rounded-lg text-black bg-white focus:ring-2 focus:ring-blue-500 outline-none"
              placeholder={type === 'personal' ? '例：平沢' : '例：平沢'}
            />
          </div>
          ...
          <div>
            ...
            <input 
              ...
              value={date}
              min={today}
              // ★ 個人練の時だけ、カレンダーで選べる一番先の日付を「明日」までに制限する！
              max={type === 'personal' ? tomorrow : undefined}
              onChange={(e) => setDate(e.target.value)}
              className="w-full p-3 text-lg border-2 border-gray-300 rounded-lg bg-white text-black focus:ring-2 focus:ring-blue-500 outline-none"
            />
            {type === 'personal' && (
              ...
              <div className={`grid grid-cols-2 gap-4 transition-opacity ${isAllDay ? 'opacity-30 pointer-events-none' : 'opacity-100'}`}>
                <div>
                  ...
                  <input 
                    ...
                    className="w-full p-3 text-lg border-2 border-gray-300 rounded-lg bg-white text-black text-center focus:ring-2 focus:ring-blue-500 outline-none"
                  />
                </div>
                <div>
          ...
          <button 
            ...
            className={`mt-4 text-white font-bold text-xl py-4 px-4 rounded-xl transition w-full shadow-md ${
              isSubmitting ? 'bg-gray-400 opacity-50 cursor-not-allowed' :
              type === 'personal' ? 'bg-green-600 hover:bg-green-700' :
