...
import { useRouter } from 'next/navigation' // ★1. 自動更新の魔法をインポート！

export default function BookingForm() {
  const router = useRouter() // ★2. 魔法の杖を準備！
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        // ★3. ここが超重要！予約が成功したら、カレンダーの最新データを裏で取ってくる！
        router.refresh() 
      }
    } finally {
      ...
      <form onSubmit={handleSubmit} className="mb-8 p-6 bg-white border border-gray-300 rounded-xl shadow-sm">
        ...
        <div className="grid gap-6">
          ...
          <div>
            ...
            <input type="text" value={bandName} onChange={(e) => setBandName(e.target.value)} className="w-full p-3 text-lg border-2 border-gray-300 rounded-lg text-black bg-white" placeholder={type === 'normal' ? '例：放課後ティータイム' : '例：クリスマスライブ'} />
          </div>
          <div>
            <label className="block font-bold mb-2 text-black">{type === 'normal' ? '代表者' : '登録者'}</label>
            <input type="text" value={leader} onChange={(e) => setLeader(e.target.value)} className="w-full p-3 text-lg border-2 border-gray-300 rounded-lg text-black bg-white" placeholder="例：平沢" />
          </div>
          <div>
            <label className="block font-bold mb-2 text-black">対象日</label>
            <input type="date" value={date} min={today} onChange={(e) => setDate(e.target.value)} className="w-full p-3 text-lg border-2 border-gray-300 rounded-lg bg-white text-black" />
          </div>
          {type !== 'event' && (
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 animate-in fade-in zoom-in duration-300">
              ...
              <div className={`grid grid-cols-2 gap-4 transition-opacity ${isAllDay ? 'opacity-30 pointer-events-none' : 'opacity-100'}`}>
                <div>
                  ...
                  <input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} className="w-full p-3 text-lg border-2 border-gray-300 rounded-lg bg-white text-black text-center" />
                </div>
                <div>
                  <label className="block font-bold mb-2 text-black">終了</label>
                  <input type="time" value={endTime} onChange={(e) => setEndTime(e.target.value)} className="w-full p-3 text-lg border-2 border-gray-300 rounded-lg bg-white text-black text-center" />
                </div>
              </div>
            </div>
          )}
          ...
          <button type="submit" disabled={isSubmitting} className={`mt-4 text-white font-bold text-xl py-4 px-4 rounded-xl transition w-full shadow-md ${isSubmitting ? 'bg-gray-400 opacity-50 cursor-not-allowed' : type === 'event' ? 'bg-purple-600 hover:bg-purple-700' : type === 'normal' ? 'bg-blue-700 hover:bg-blue-800' : type === 'live' ? 'bg-red-500 hover:bg-red-600' : 'bg-gray-700 hover:bg-gray-800'}`}>
