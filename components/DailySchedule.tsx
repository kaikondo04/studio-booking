...
export default function DailySchedule({ date, bookings }: { date: string, bookings: Booking[] }) {
  ...
  const getBookingAt = (hour: number) => {
    return bookings.find(b => {
      ...
      if (hour === endH && end.getMinutes() > 0) return true
      return false
    })
  }
  ...
  return (
    <div className="mb-10 border rounded-xl overflow-hidden shadow-sm bg-white">
      <div className="bg-gray-900 text-white p-3 font-bold text-lg text-center">{date}</div>
        ...
        <div className="bg-purple-50 border-b border-purple-100">
          {eventBookings.map(event => (
            <div key={event.id} className="p-2 flex justify-between items-center border-b last:border-b-0 border-purple-200/50">
              ...
              <div className="flex-shrink-0 ml-2"><DeleteButton id={event.id} /></div>
            </div>
          ))}
        </div>
      )}
          ...
          return (
            <div key={hour} className="flex bg-white h-24 relative">
              ...
              <div className="flex-grow relative w-full">
                ...
                {booking && style ? (
                  <div className={`absolute left-1 right-1 rounded-md border overflow-hidden shadow-sm z-10 block transition-all ${isSpecial ? 'bg-red-100 border-red-400' : 'bg-blue-50 border-blue-200'}`} style={{ top: style.top, height: style.height }}>
                    {isStartBlock && (
                      <div className="p-1 leading-tight">
                        ...
                        <div className="font-bold text-black text-sm truncate">{isSpecial ? '⛔ ' : ''}{booking.band_name}</div>
                  ...
                  <div className="h-full flex items-center pl-2"><span className="text-gray-300 text-sm">Empty</span></div>
                )}
              </div>
            </div>
          )
